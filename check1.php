...
if(isset($_POST["usernamecheck"])){
	...
	$sql = "SELECT * FROM reg_details WHERE idno='$username' LIMIT 1";
    $query = mysqli_query($con,$sql); 
    $uname_check = mysqli_num_rows($query);

    if (strlen($username) < 8 || strlen($username) > 8) {
	    echo '<strong style="color:#F00;">8 digits please</strong>';
	    exit();
    }
    if ($uname_check < 1) {
	    ...
	    echo '<strong style="color:#F00;">' . $username . ' is in use</strong>';
	    exit();
    }
}
